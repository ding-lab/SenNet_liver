...
cell.types.oi %>% walk (function(ct) {
  ...
  if(!file.exists(paste0(add_filename,"_",make.names(ct), ".rds"))) {
    ...
    ggsave(glue::glue("Dimplot_{add_filename}_{ct.name}_Patient.id.pdf"), width = 8, height = 5)
    
    DimPlot(int.sub, reduction='umap.harmony', group.by = 'seurat_clusters')
    ggsave(glue::glue("Dimplot_{add_filename}_{ct.name}_seurat_clusters.pdf"), width = 6.5, height = 5)
