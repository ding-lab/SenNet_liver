...
runHarmonyNormalization <- function(obj, dims=30, column = 'Mouse_ID') {
  ...
  obj <- obj %>%
    RunHarmony(group.by.vars = column, reduction = 'pca', assay.use = 'SCT') %>%
...
panc.my <- runHarmonyNormalization(panc.my, column = c('Mouse_ID', 'Cohort'))

saveRDS(panc.my,  paste0(add_filename,".rds"))

DimPlot(panc.my, group.by = 'Mouse_ID', reduction='umap.harmony', label = TRUE, raster = FALSE)
ggsave(glue::glue("Dimplot_Mouse_ID_{add_filename}_umap.harmony.pdf"),
       height=5,width=8,useDingbats=FALSE,limitsize = FALSE)

DimPlot(panc.my, group.by = 'seurat_clusters', reduction='umap.harmony', label = TRUE, raster = FALSE)
